...
// --- Main Application ---
            ...
            if (typeof window.firebaseRegisterSession === 'function') {
                ...
                try {
                    ...
                    if (result.blocked) {
                        ...
                        if (errEl) {
                            errEl.style.display = 'block';
                            errEl.textContent = 'Ya hay una sesión activa en otro dispositivo. Cerrá sesión allá primero.';
                        }
        ...
        // URLs y token cargados desde config.js (no esté en el repo)
        ...
        async function authFetch(url, options = {}, _retryCount = 0) {
            ...
            try {
                ...
                clearTimeout(timer);
                if (res.status === 401) {
                    logError(`Error 401 en ${url.split('/').pop()} — sesión expirada o JWT inválido.`);
                ...
                if (_retryCount === 0 && (e.message.includes('Failed to fetch') || e.name === 'TypeError')) {
                    logError(`Error de red en ${url.split('/').pop()}, reintentando en 2s...`);
        ...
        document.querySelectorAll('.nav-item').forEach(i => i.onclick = () => {
            // Verificar que el tab esté permitido para el rol actual
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    try {
                        ...
                        logError(`Error parseando respuesta de abogados: ${parseErr.message}`);
                    }
                    ...
                    if (container) container.innerHTML = `<p style="color:#ef4444;font-size:0.85rem;text-align:center;padding:2rem;">No se pudo conectar con n8n (${res.status}).</p>`;
                }
            } catch(e) {
                logError(`Error al cargar abogados: ${e.message}`);
                if (container) container.innerHTML = `<p style="color:#ef4444;font-size:0.85rem;text-align:center;padding:2rem;">Sin conexión con n8n.</p>`;
            }
        }
...
async function loadRealData() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    try {
                        ...
                        logError(`Respuesta no es JSON válido: ${parseErr.message}`);
                        el('dash-rows').innerHTML = `<tr><td colspan="6" style="text-align:center;color:#ef4444;padding:1.5rem;font-size:0.85rem;">Respuesta inválida de n8n.</td></tr>`;
                        const _expParse = document.getElementById('exp-cards-container');
                        if (_expParse) _expParse.innerHTML = `<div class="empty-state" style="color:#ef4444;">Error de formato.</div>`;
                        return;
                    }
                    ...
                    el('dash-rows').innerHTML = `<tr><td colspan="6" style="text-align:center;color:#ef4444;padding:1.5rem;font-size:0.85rem;">Error al conectar con n8n (${res.status}). Reintenta.</td></tr>`;
                    const _expErr = document.getElementById('exp-cards-container');
                    if (_expErr) _expErr.innerHTML = `<div class="empty-state" style="color:#ef4444;">Error de sincronización.</div>`;
                    logError(`Error de sincronización: HTTP ${res.status}`);
                }
            } catch(e) {
                el('dash-rows').innerHTML = `<tr><td colspan="6" style="text-align:center;color:#ef4444;padding:1.5rem;font-size:0.85rem;">Sin conexión con n8n.</td></tr>`;
                const _expErr2 = document.getElementById('exp-cards-container');
                if (_expErr2) _expErr2.innerHTML = `<div class="empty-state" style="color:#ef4444;">Sin conexión.</div>`;
        ...
        function updateUI() {
            ...
            if (conVenc.length) {
                el('dash-deadlines').innerHTML = conVenc.slice(0, 5).map(c => {
                    ...
                    const badge = diff <= 0 ? 'VENCIDO' : diff <= 3 ? `${diff}d` : diff <= 10 ? `${diff} dias` : c.venc;
                    ...
                    <div class="asig-card">
                        ...
                        <div class="asig-actions">
                            ...
                            <div style="flex-grow:1;display:flex;gap:12px;align-items:center;flex-wrap:wrap;">
                                ...
                                <button class="btn-asig-main" onclick="finish('${tok}')"> Confirmar Asignación</button>
                            </div>
                        </div>
                ...
                if (response.ok) {
                    ...
                    try {
                        ...
                        if (clientRes.ok) {
                            ...
                            log(` Aviso: Respuesta inesperada al guardar cliente (${clientRes.status}).`);
                        }
                    } catch(e) {
                        log(` Aviso: No se pudo guardar cliente en directorio.`);
                    }
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:6px;">
                        <div style="display:flex;align-items:center;gap:6px;flex-wrap:wrap;">
                            ...
                            <span style="font-size:0.62rem;font-weight:700;color:${isPublic ? '#22c55e' : 'var(--silver)'};"
                                  title="${isPublic ? 'Visible en portal' : 'Solo interno'}">${isPublic ? 'Portal' : 'Interno'}</span>
                        </div>
            ...
            modal.innerHTML = `
                <div style="background:#0c0f16;border:1px solid rgba(201,168,76,0.3);border-radius:12px;
                    ...
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
                        <span style="color:var(--gold);font-weight:700;font-size:0.9rem;"> Editar Observación</span>
        ...
        async function fetchClientMails(tok, clientName) {
            ...
            // Toggle: si ya esté abierto, cerrar
            ...
            if (!clientEmail) {
                ...
                panel.innerHTML = `<p style="margin:0;font-size:0.82rem;color:#f97316;"> Ingresá el email del cliente en el campo de arriba para buscar sus correos.</p>`;
                return;
            }
        ...
        function waitForFirebaseAndRender() {
            ...
            function tryRender() {
                ...
                // Si Firebase esté listo podemos hacer get
                ...
                // Todavía no esté listo — reintentar hasta 10 veces
                if (attempts < 10) {
        ...
        function renderActiveSessionsLive(sessions) {
            ...
            container.innerHTML = Object.entries(sessions).map(([username, data]) => {
                ...
                const devIcon     = ua.includes('Mobile') ? '' : '';
        ...
        async function renderActiveSessions() {
            ...
            // Usar cache en tiempo real si ya esté disponible
            if (window._firebaseSessions !== undefined) {
                renderActiveSessionsLive(window._firebaseSessions);
                return;
            }
        ...
        async function pingEngineAdmin() {
            ...
            try {
                ...
                if (res.ok) {
                    ...
                    logError(` Health check respondió ${res.status} — revisar servicios en n8n`);
                }
            } catch(e) {
        ...
        function renderObservaciones(list) {
            ...
            tbody.innerHTML = list.map(o => {
                ...
                const isPublic = vis === 'Público' || vis === 'Publico';
                const eyeIcon = isPublic ? '<span title="Visible por el cliente"></span>' : '<span title="Nota Interna Privada"></span>';
        ...
        function renderCasoCard(c, clienteNombre, idx) {
            ...
            <div class="exp-card" id="exp-card-${cardId}" style="margin-bottom:10px;">
                ...
                <div class="exp-card-meta" style="margin-top:8px;">
                    <span class="exp-meta-chip"> ${esc(c.lawyer || 'Sin abogado')}</span>
        ...
        function renderAnotaciones(tok, list) {
            ...
            el.innerHTML = list.map(a => {
                ...
                const visLabel = isPublic
                    ? `<span style="font-size:0.65rem;font-weight:700;color:#22c55e;letter-spacing:0.5px;">Portal</span>`
        ...
        function abrirEditarAnotacion(id, tok) {
            ...
            modal.innerHTML = `
                <div style="background:#1a1a1a;border:1px solid rgba(201,168,76,0.3);border-radius:10px;padding:20px;width:90%;max-width:480px;">
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
                        <span style="color:var(--gold);font-weight:700;font-size:0.9rem;"> Editar Anotación</span>
                ...
                <div style="display:flex;gap:12px;align-items:flex-start;flex-wrap:wrap;">
                    ...
                    <div style="${colStyle}border-left:3px solid rgba(34,197,94,0.4);">
                        <p style="margin:0;font-size:0.7rem;font-weight:700;color:#22c55e;
                                   text-transform:uppercase;letter-spacing:0.8px;"> Notas del Portal</p>
